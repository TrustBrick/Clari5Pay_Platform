 ...
 *  auth token is never sent to the third-party API). Returns null on invalid/offline. */
export const lookupIfsc = async (ifsc: string): Promise<IfscInfo | null> => {
  ...
  try {
    ...
    return { bank: d.BANK, branch: d.BRANCH, address: d.ADDRESS, city: d.CITY, state: d.STATE };
  } catch {
    return null; // offline / blocked — caller falls back to manual entry
  }
};
