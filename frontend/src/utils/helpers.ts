...
/** The timestamp shape used in DOWNLOADED FILES — CSV, XLSX and the printed/PDF reports.
 *
 *  Deliberately NOT the display format. A file is read by machines as often as by people: the
 *  export shape keeps seconds, and keeps the locale output these files have always carried, so a
 *  spreadsheet or a downstream import that already parses them keeps working untouched. Changing
 *  how a screen looks is safe; silently changing the contents of a file someone else's process
 *  reads is not.
 *
 *  On-screen timestamps use `formatDateTime` instead.
 */
export const formatDateTimeExport = (d?: string | number | Date | null) => {
  if (!d) return '—';
  const dt = toDate(d);
...
/** IST with the zone stated: "04 Sep 2026, 04:06 PM IST". Unchanged behaviour, standard shape. */
