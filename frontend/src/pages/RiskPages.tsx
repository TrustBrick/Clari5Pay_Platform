...
import { fmt, fileToDataUrl, memberLabel, formatDateTime, formatDate } from '../utils/helpers';
...
// ── Investigation-report Excel (.xlsx) — mirrors the PDF's data ──
export function exportRiskXlsx(p: RiskProfile, generatedBy?: string) {
  const pr = p.profile;
  const rel = p.relationships;
  const recs = riskRecs(pr.riskLevel, rel.relatedMemberships.length > 0);
  const kv = (k: string, v: unknown) => ({ field: k, value: v == null ? '' : String(v) });
  const stat = (label: string, s: RiskProfile['txnIntel']['deposits']) =>
    ({ kind: label, total: s.total, largest: s.largest, average: s.average, count: s.count });
  const findings = [
    ...p.summary.indicators.map(x => ({ category: 'Risk Indicator', detail: x })),
    ...p.summary.strengths.map(x => ({ category: 'Strength', detail: x })),
    ...recs.map(x => ({ category: 'Recommendation', detail: x })),
  ];
  downloadXlsx(`risk-assessment-${pr.memberId}.xlsx`, [
    { name: 'Member Info', columns: [
        ...
        kv('Linked Bank Accounts', rel.linkedAccounts.length), kv('Linked UPI IDs', rel.linkedUpis.length),
        kv('Repeated Senders', rel.repeatedSenders.length), kv('Related Memberships', rel.relatedMemberships.length),
        kv('Generated By', generatedBy || '—'),
      ] },
    { name: 'Transaction Summary', columns: [
        { header: 'Type', get: (r: any) => r.kind },
        { header: 'Total', get: (r: any) => r.total, width: 14 },
        { header: 'Largest', get: (r: any) => r.largest, width: 14 },
        { header: 'Average', get: (r: any) => r.average, width: 14 },
        { header: 'Count', get: (r: any) => r.count },
      ], rows: [
        stat('Deposits', p.txnIntel.deposits), stat('Withdrawals', p.txnIntel.withdrawals), stat('Settlements', p.txnIntel.settlements),
      ] },
    { name: 'Findings', columns: [
        { header: 'Category', get: (r: any) => r.category, width: 18 },
        { header: 'Detail', get: (r: any) => r.detail, width: 60 },
      ], rows: findings },
  ]);
}
...
export function exportComplaintPdf(args: { caseId?: string; memberId: string; memberName: string; merchantName: string; bank: BankDetail; description: string; documents: PdfDoc[]; status?: string; priority?: string; riskLevel?: string; timeline?: PdfTimeline }) {
  ...
  const fdt = (s?: string | null) => formatDateTime(s);
  ...
  <div class="sign"><div class="muted">Place: ______________<br>Date: ${formatDate(new Date())}</div><div class="muted" style="text-align:right">Yours faithfully,<br><br>______________________<br>Complainant Signature</div></div>
...
// ── Cyber Crime Complaint Excel (.xlsx) — case fields + timeline ──
export function exportComplaintXlsx(args: { caseId?: string; memberId: string; memberName: string; merchantName: string; bank: BankDetail; description: string; documents: PdfDoc[]; status?: string; priority?: string; riskLevel?: string; timeline?: PdfTimeline }) {
  const b = args.bank;
  const fdt = (s?: string | null) => formatDateTime(s, { empty: '' });
  const tl = args.timeline || {};
  const kv = (k: string, v: unknown) => ({ field: k, value: v == null ? '' : String(v) });
  downloadXlsx(`cyber-complaint-${args.caseId || args.memberId}.xlsx`, [
    { name: 'Complaint', columns: [
        { header: 'Field', get: (r: any) => r.field, width: 24 },
        ...
        kv('Bank Name', b.bankName), kv('Branch', b.branch), kv('IFSC Code', b.ifsc), kv('UPI ID', b.upiId),
        kv('Complaint Details', args.description),
        kv('Documents Enclosed', args.documents.map(d => d.name).join('; ')),
      ] },
    { name: 'Case Timeline', columns: [
        { header: 'Stage', get: (r: any) => r.stage, width: 18 },
        { header: 'Timestamp', get: (r: any) => r.at, width: 22 },
        { header: 'By', get: (r: any) => r.by, width: 22 },
      ], rows: [
        { stage: 'Opened', at: fdt(tl.openedAt), by: tl.openedBy || '' },
        { stage: 'Under Review', at: fdt(tl.underReviewAt), by: tl.underReviewBy || '' },
        { stage: 'Escalated', at: fdt(tl.escalatedAt), by: tl.escalatedBy || '' },
        { stage: 'Complaint Filed', at: fdt(tl.complaintFiledAt), by: tl.complaintFiledBy || '' },
        { stage: 'Closed', at: fdt(tl.closedAt), by: tl.closedBy || '' },
      ] },
  ]);
}
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: 16, borderRadius: 12, background: RISK_META[p.profile.riskLevel].bg, flexWrap: 'wrap' }}>
            <div>
              ...
              <p style={{ margin: '4px 0 0', fontSize: 11, color: T.textMuted }}>Generated {formatDateTime(new Date())} · By: {genBy}</p>
            </div>
