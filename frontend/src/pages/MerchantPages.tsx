...
export const SettlementForm: React.FC<{ user: User; onSubmitted?: () => void }> = ({ user, onSubmitted }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const created = await transactionAPI.createSettlement(payload);
      // Phase 4 (demo): assign the chosen Non-EPS agent + account. Two separate operations by
      // design (create endpoint untouched) — see the "known limitation" note in agent_assignment.py.
      if (IS_DEMO && assign.agentId && assign.accountId && created?.ref) {
        try { await agentAssignmentAPI.assign(created.ref, { agentId:Number(assign.agentId), agentAccountId:Number(assign.accountId), paymentMethod:assign.accountType }); }
...
const ManagementPage: React.FC<{
  ...
  // Group by Membership ID, most requests first. Settlements are the one exception: they are paid
  // to the merchant/company itself and carry no membership, so they group under the company name
  // rather than all falling through to 'Unassigned'. Deposits and withdrawals are unaffected.
  const groups = Object.values(
    mine.reduce((acc, t) => {
      const memberKey = t.memberId || t.member || 'Unassigned';
      const key = byCompany ? (t.merchant || memberKey) : memberKey;
    ...
    <div>
      ...
      <Card>
        <div style={{ padding:'16px 20px',borderBottom:`1px solid ${T.border}` }}>
          <h3 style={{ margin:0,fontSize:14,fontWeight:800 }}>{groupCardTitle} ({groups.length})</h3>
        </div>
              ...
              <tbody>
                ...
                {groups.map((g,i)=>(
                  <tr key={g.key} style={{ background:i%2===0?T.surface:'#f8faff',cursor:'pointer' }} onClick={()=>setOpenMember(g.key)}>
                    <td style={{ padding:'11px 14px',fontWeight:700,color:T.textMain }}>{byCompany ? g.key : memberLabel(g.key, g.items[0]?.member)}</td>
      ...
      {active && (
        <Modal title={`${byCompany ? '' : 'Member '}${active.key} — ${noun} History`} onClose={()=>setOpenMember(null)} xl>
