...
import type { Transaction, User, Account, AccountBalance, AccountLedgerEntry, AllocationDecision, PayoutAllocation, AccountUsers, AccountUser, MerchantBalance, MerchantStats, GlobalSummary, AdminUpi, ActiveUsersData, ReportRow } from '../types';
...
const RequestModal: React.FC<{
  ...
  // An OVERRIDE, not a selection. Blank means "pay the account(s) the engine allocated", which is
  // the normal path; setting it directs the payout elsewhere, and the backend puts that account
  // through the identical eligibility rules before it will debit anything.
  const [payoutAccountRef, setPayoutAccountRef] = useState('');
  const [overrideAccount, setOverrideAccount] = useState(false);
  const [payoutInfo, setPayoutInfo] = useState<PayoutAllocation | null>(null);
  ...
  // Pay merchant → upload receipt → Completed. ACCOUNT_SUBMITTED is where an approved withdrawal
  // now lands: the payout allocation engine has already named the account(s) that pay it, so this
  // step PAYS them. ACCOUNT_REQUESTED stays payable for settlements and for any withdrawal raised
  // before automatic allocation existed.
  const payStep = canAct && !isDeposit
    && (tx.status === 'SLIP_SUBMITTED' || tx.status === 'ACCOUNT_REQUESTED' || tx.status === 'ACCOUNT_SUBMITTED');
  // The withdrawal exception: the engine could not place this one. This is the ONE case that still
  // needs an Admin, and the intended fix is to remove the cause and retry the ENGINE — not to
  // assign an account by hand.
  const payoutUnplaced = canAct && isWithdrawal && tx.status === 'NO_ELIGIBLE_ACCOUNT';
  ...
  // The account(s) the engine allocated. On the normal path these are simply paid; an operator is
  // only asked to name one when a withdrawal has none — an older row, or one whose allocation
  // found nothing and has not been retried.
  const allocatedLegs = (payoutInfo?.legs || []).filter(l => l.status !== 'RELEASED');
  const needsManualAccount = needsPayout && payMethod === 'BANK' && allocatedLegs.length === 0;
  ...
  // The withdrawal's payout allocation — which account(s) the engine chose, and (when it could
  // choose none) why. Admin-only, so the capacity figures behind the decision never travel on a
  // merchant payload.
  useEffect(() => {
    if (!isWithdrawal) { setPayoutInfo(null); return; }
    transactionAPI.payoutAllocation(tx.id).then(setPayoutInfo).catch(() => setPayoutInfo(null));
  }, [isWithdrawal, tx.id, tx.status]);

  const retryPayout = async () => {
    setSaving(true);
    try {
      await transactionAPI.retryPayoutAllocation(tx.id);
      showToast(`${tx.ref} — allocation re-run`);
      onDone?.(); onClose();
    } catch (e: any) {
      showToast(e?.response?.data?.detail || 'Allocation failed', 'error');
    } finally { setSaving(false); }
  };
  ...
  const complete = async (withReceipt: boolean) => {
    ...
    if (needsPayout) {
      // No account needs picking on the normal path — the engine allocated one when the request
      // was raised. An account is only required when the operator has chosen to override it, or
      // when this is an older withdrawal that was never allocated.
      if (payMethod === 'BANK' && needsManualAccount && !payoutAccountRef) {
        showToast('Select the payout account', 'error'); return;
      }
      if (payMethod === 'MANUAL' && !manualRef.trim()) { showToast('Enter the manual payment reference', 'error'); return; }
      if (payMethod === 'BANK' && payoutAccountRef && selectedPayoutBal != null && selectedPayoutBal < tx.amount) {
    ...
    try {
      ...
      await transactionAPI.markDone(tx.id, withReceipt ? {
        ...
        ...(needsPayout ? {
          ...
          // Sent ONLY as an override. Left out on the normal path so the backend pays exactly what
          // it allocated — the browser never nominates the paying account for an ordinary payout.
          payoutAccountRef: payMethod === 'BANK' && payoutAccountRef ? payoutAccountRef : undefined,
  ...
  return (
    <Modal title={`${title} — ${tx.ref}`} onClose={onClose} wide>
      ...
      {/* The WITHDRAWAL allocation exception. The ONE withdrawal case that still needs an Admin:
          the engine could not place this payout. The fix is to remove the cause — raise a Highest
          Debit, activate an account, enable a transaction mode, fund an account — and re-run the
          ENGINE. The Admin never picks the account; they clear what is blocking it. */}
      {payoutUnplaced && (
        <div style={{ marginTop:18,paddingTop:16,borderTop:`1px solid ${T.border}` }}>
          <div style={{ background:T.warningBg,border:`1px solid ${T.warning}`,borderRadius:10,padding:'12px 14px',marginBottom:12 }}>
            <p style={{ margin:0,fontSize:12.5,fontWeight:800,color:T.textMain }}>
              <Icon name="warning" size={14} /> No Eligible Payout Account
            </p>
          ...
          <Btn size="sm" onClick={retryPayout} disabled={saving}>
            {saving ? 'Allocating…' : <><Icon name="refresh" size={13} /> Retry Automatic Allocation</>}
          </Btn>
        </div>
      )}

      {/* The DEPOSIT allocation exception. This is the ONE deposit case that still needs an Admin,
          and the fix is configuration — raise a Highest Credit, activate an account, add one —
          followed by a retry. Choosing an account by hand is offered underneath as the last
      ...
      {/* PAYOUT DETAILS ── the account(s) the money leaves, or an offline payment.
          The account is NOT chosen here. The withdrawal allocation engine picked it when the
          request was raised, applying the account's availability, its supported transaction modes,
          its available balance and its hard daily debit limit; this step pays what it picked. The
          override below exists for the cases the engine cannot know about, and it is not a way
          round any of those rules — the backend puts an overriding account through the identical
          checks and refuses one that fails them. */}
      {needsPayout && (
        <div style={{ marginTop:18,paddingTop:16,borderTop:`1px solid ${T.border}` }}>
          ...
          {payMethod === 'BANK' ? (
            <>
              ...
              <PayoutAllocationPanel
                ...
                emptyNote="This withdrawal has no allocated payout account — select one below, or re-run the automatic allocation." />

              {/* The escape valve. Closed by default, because the engine's choice is the answer
                  on every ordinary payout and re-opening the question per request is the manual
                  step this feature removes. */}
              {needsManualAccount ? null : (
                <Btn size="sm" variant="ghost" onClick={()=>{ setOverrideAccount(o=>!o); setPayoutAccountRef(''); }}
                     style={{ marginBottom:12 }}>
                  <Icon name="edit" size={13} /> {overrideAccount ? 'Use the allocated account' : 'Pay from a different account'}
                </Btn>
              )}

              {(needsManualAccount || overrideAccount) && (
                <>
                  <Sel label="Payout Account" required={needsManualAccount} value={payoutAccountRef}
                    onChange={e=>setPayoutAccountRef(e.target.value)}
                    options={[{ value:'', label: needsManualAccount ? '— Select Bank Account —' : '— Keep the allocated account —' }, ...accounts.map(a => ({
                  ...
                  <p style={{ fontSize:11,color:T.textMuted,margin:'-6px 0 12px' }}>
                    Validated server-side against the same availability, transaction-mode, balance
                    and daily debit-limit rules the automatic allocation applies — an account that
                    fails any of them is refused.
                  </p>
                </>
              )}
            </>
            ...
            <>
              <Input label="Manual Payment Reference" required value={manualRef} onChange={e=>setManualRef(e.target.value)}
                ...
                hint="An offline payment is not tied to a payout bank account; this reference is what makes it traceable. Any allocated payout capacity is released back to the account." />
            </>
          )}
          ...
          <div style={{ background:T.canvas,borderRadius:10,padding:'10px 14px',marginBottom:4 }}>
            ...
            {payMethod === 'BANK' && payoutAccountRef && selectedPayoutAcc && (
              <Row k="Payout Account (override)" v={<span>{selectedPayoutAcc.bankName}<br /><span style={{ fontWeight:600,color:T.textMuted }}>A/C {maskAccount(selectedPayoutAcc.accountNumber)}</span></span>} />
            )}
            {payMethod === 'BANK' && payoutAccountRef && selectedPayoutBal != null && (
              <Row k="Account Balance" v={<span style={{ color: selectedPayoutBal < tx.amount ? T.danger : T.textMain }}>{fmt(selectedPayoutBal)}</span>} />
            )}
            {payMethod === 'BANK' && !payoutAccountRef && allocatedLegs.length > 1 && (
              <Row k="Accounts Debited" v={`${allocatedLegs.length} accounts`} />
            )}
            <Row k="Amount to Debit" v={payMethod === 'BANK' ? fmt(tx.amount) : `${fmt(tx.amount)} (offline)`} />
          </div>
          {payMethod === 'BANK' && payoutAccountRef && selectedPayoutBal != null && selectedPayoutBal < tx.amount && (
      ...
      {payStep && (
        <div style={{ marginTop:18,paddingTop:16,borderTop:`1px solid ${T.border}` }}>
          ...
          <div style={{ display:'flex',gap:10,marginTop:12 }}>
            <Btn onClick={()=>complete(true)} disabled={saving||(needReceipt&&!receipts.length)||(needUtr&&!payUtr.trim())||(needsManualAccount&&!payoutAccountRef)||(needsPayout&&payMethod==='MANUAL'&&!manualRef.trim())}>{saving ? 'Saving...' : <><Icon name="approve" size={14} /> Complete</>}</Btn>
            <Btn variant="secondary" onClick={onClose}>Cancel</Btn>
          </div>
        </div>
      )}
...
export const AdminAccountsPage: React.FC = () => {
  ...
  // Payout capability editing, inside the Details popup. Each toggle is its own audited PATCH —
  // the backend validates every mode against the platform's four and rejects anything else.
  ...
  // The platform's four payout transaction modes. Not a new list — these are the same modes the
  // deposit side already uses, and the backend validates against its own copy, so a value typed
  // into a request could never widen it.
  const PAYOUT_MODE_OPTIONS = ['UPI', 'IMPS', 'NEFT', 'RTGS'];

  const savePayoutModes = async (mode: string, enabled: boolean) => {
    if (!detail) return;
    const current = detail.payoutModes && detail.payoutModesConfigured
      ? detail.payoutModes
      : PAYOUT_MODE_OPTIONS;
    const next = enabled ? [...new Set([...current, mode])] : current.filter((m: string) => m !== mode);
    setSavingModes(true);
    try {
      // An empty list is sent as-is and stored as "every mode" — the unconfigured state. Turning
      // the last one off therefore restores full capability rather than taking the account out of
      // service, which is what the backend means by NULL.
      const updated = await accountAPI.updatePayoutModes(detail.referenceNumber, next);
      setDetail(updated);        // the server's value, not the one clicked
      reload();
      showToast(next.length ? `Payout modes: ${next.join(', ')}` : 'Payout modes: all');
    } catch (e: any) {
      showToast(e?.response?.data?.detail || 'Failed to update the payout modes', 'error');
          ...
          <table style={{ width:'100%',borderCollapse:'collapse',fontSize:12 }}>
            <thead>
              <tr style={{ background:T.canvas }}>
                ...
                {['Account Name','Account Number','IFSC Code','Branch','Highest Credit','Used Today','Remaining','Highest Debit','Debit Used','Debit Left','Available','Users','Status','Details'].map(h=>(
            ...
            <tbody>
              ...
              {filtered.map((a,i)=>{ const bal = balMap[a.referenceNumber]; return (
                <tr key={a.id} style={{ background:i%2===0?T.surface:'#f8faff' }}>
                  ...
                  {/* The debit side of the same daily story: what this account has paid out today
                      and what it may still pay. Both come from the withdrawal allocation engine's
                      own functions, so the screen and the engine cannot disagree. */}
                  <td style={{ padding:'11px 14px',fontWeight:700 }}>{fmt(bal?.debitUsedToday ?? 0)}</td>
                  <td style={{ padding:'11px 14px',fontWeight:800,color: (bal?.remainingDebit ?? 0) > 0 ? T.blue : T.danger }}>{fmt(bal?.remainingDebit ?? 0)}</td>
          ...
          <div style={{ marginTop:18,paddingTop:14,borderTop:`1px solid ${T.border}` }}>
            ...
            {!editLimits ? (
              <>
                ...
                {/* ── Today's position against the daily DEBIT limit ──
                    Highest Debit is a HARD DAILY CEILING on the way out, exactly as Highest
                    Credit is on the way in: a withdrawal is only paid from here while Used Today
                    plus the payout stays within it. Capacity is consumed the moment a withdrawal
                    is ALLOCATED, not when it is paid, which is what stops two requests being
                    given the same headroom. Display only — the backend recomputes every figure
                    under a row lock before it allocates anything. */}
                {(() => {
                  const b = balMap[detail.referenceNumber];
                  const limit = detail.highestDebit ?? 0;
                  const used = b?.debitUsedToday ?? 0;
                  const left = b?.remainingDebit ?? Math.max(0, limit - used);
                  const reserved = b?.reservedForPayouts ?? 0;
                  const pct = limit > 0 ? Math.min(100, (used / limit) * 100) : 0;
                  const full = limit > 0 && left <= 0;
                  return (
                    <div style={{ marginTop:12,background:T.canvas,borderRadius:10,padding:'12px 14px' }}>
                      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'baseline',gap:10,marginBottom:8 }}>
                        <span style={{ fontSize:11,fontWeight:800,color:T.textMuted,textTransform:'uppercase',letterSpacing:'0.05em' }}>Today's Debit Usage</span>
                        <span style={{ fontSize:11,color:T.textMuted }}>{b?.payoutsToday ?? 0} payout{(b?.payoutsToday ?? 0) === 1 ? '' : 's'} today</span>
                      </div>
                      <div style={{ height:6,borderRadius:3,background:T.border,overflow:'hidden',marginBottom:10 }}>
                        <div style={{ width:`${pct}%`,height:'100%',background: full ? T.danger : T.blue,transition:'width .3s' }} />
                      </div>
                      <Row k="Debit Used Today" v={fmt(used)} />
                      <Row k="Remaining Debit" v={<span style={{ color: full ? T.danger : T.textMain }}>{fmt(left)}</span>} />
                      {reserved > 0 && (
                        <Row k="Reserved for Payouts" v={<span title="Allocated to withdrawals that have not been paid yet. No money has moved, so it is not deducted from the balance.">{fmt(reserved)}</span>} />
                      )}
                      {limit <= 0 && (
                        <p style={{ margin:'8px 0 0',fontSize:11,color:T.danger }}>
                          No Highest Debit configured — this account is never allocated a withdrawal. Set a limit above.
                        </p>
                      )}
                      {full && (
                        <p style={{ margin:'8px 0 0',fontSize:11,color:T.danger }}>
                          Daily debit limit reached — no further withdrawal is paid from here until tomorrow.
                        </p>
                      )}
                    </div>
                  );
                ...
                {/* ── Payout capability ──
                    Which transaction modes this account can SEND by. The withdrawal allocation
                    engine excludes an account that cannot process a request's mode, so this is a
                    financial control and the API audits every change. An account with none
                    configured supports every mode — an empty capability read as "supports
                    nothing" would take every account out of service. */}
                <div style={{ marginTop:12,background:T.canvas,borderRadius:10,padding:'12px 14px' }}>
                  <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,marginBottom:8 }}>
                    <span style={{ fontSize:11,fontWeight:800,color:T.textMuted,textTransform:'uppercase',letterSpacing:'0.05em' }}>Payout Modes</span>
                    {canEditLimits && (
                      <Btn size="sm" variant="ghost" disabled={savingModes}
                        onClick={()=>setEditModes(m=>!m)}>
                        <Icon name="edit" size={12} /> {editModes ? 'Done' : 'Edit'}
                      </Btn>
                    )}
                  </div>
                  <div style={{ display:'flex',gap:8,flexWrap:'wrap' }}>
                    {PAYOUT_MODE_OPTIONS.map(mode => {
                      const on = (detail.payoutModes || PAYOUT_MODE_OPTIONS).includes(mode);
                      return editModes ? (
                        <label key={mode} style={{ display:'inline-flex',alignItems:'center',gap:6,fontSize:12,fontWeight:700,color:T.textMain,cursor:savingModes?'wait':'pointer' }}>
                          <input type="checkbox" checked={on} disabled={savingModes}
                            onChange={e=>savePayoutModes(mode, e.target.checked)} />
                          {mode}
                        </label>
                      ) : (
                        <span key={mode} style={{
                  ...
                  <p style={{ margin:'8px 0 0',fontSize:11,color:T.textMuted }}>
                    {detail.payoutModesConfigured
                      ? 'A withdrawal in a mode this account does not support is never paid from here.'
                      : 'Not configured — this account can pay by any mode. Narrow it above if it should not.'}
                  </p>
                </div>
              </>
            ) : (
