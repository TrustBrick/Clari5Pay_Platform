...
import { fmt, today, depositTypeLabel, memberLabel, merchantRoleLabel, clientApproverLabel, merchantApproverLabel, formatIndianAmountInput, parseIndianAmount, formatDateTime, formatIstParts, formatDateTimeExport } from '../utils/helpers';
...
const TreasuryReport: React.FC<{ rows: ReportRow[]; businessName: string; generatedBy: string; rangeLabel: string; internalView?: boolean }> =
  ({ rows, businessName, generatedBy, rangeLabel, internalView = false }) => {
    ...
    const csvRows = data.map(r => [r.ref, r.member || '', r.memberId || '', `${r.date || ''} ${r.time || ''}`.trim(), prettyStatusR(r.status), r.approvedBy ? approverCell(r, internalView) : '', opCsv(r), r.amount, methodLabel(r)]);
    const pdfRows = data.map(r => [r.ref, r.member || '—', r.memberId || '—', `${r.date || ''} ${r.time || ''}`.trim(), prettyStatusR(r.status), approverCell(r, internalView), operatorLabel(r), fmt(r.amount), methodLabel(r)]);
    const onExcel = () => {
