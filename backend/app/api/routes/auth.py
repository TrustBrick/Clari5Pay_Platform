...
def _user_to_out(u: User) -> dict:
    return {
        ...
        # RAW support-duty state: None = the Admin has never opened the Support Duty control,
        # which counts as available support while they are signed in. "OFF" is the explicit
        # opt-out. Distinct from supportAvailability above, which coerces None to "AVAILABLE" for
        # the support portal's non-null union and so cannot tell those two apart.
